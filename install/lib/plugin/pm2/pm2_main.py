...
source /www/server/nvm/nvm.sh && ''';
    ...
    def List(self,get):
        try:
            ...
            rep = "[\xe2\x8c\xe2\x94\x80\x82\xb4\x98\x90\xac\xbc\xa4\x9c]";
            t2 = re.sub(rep,'',tmp[0]).split("\n\n")
            tmpArr = t2[1];
            if t2[1].find('App name') != -1: tmpArr = t2[2];
            appList = tmpArr.split('\n');
            result = []
            tmp = public.ExecShell('lsof -i -P|grep node');
            plist = tmp[0].split('\n')
            for app in appList:
                if not app: continue;
                tmp2 = app.split();
                ...
                appInfo['restart'] = tmp2[5];
                appInfo['uptime'] = tmp2[6];
                appInfo['cpu'] = tmp2[7];
                appInfo['mem'] = tmp2[8] + ' ' + tmp2[9];
                appInfo['user'] = tmp2[10];
                appInfo['watching'] = tmp2[11];
                appInfo['port'] = 'OFF';
                appInfo['path'] = 'OFF';
                for p in plist:
                    ptmp = p.split();
    ...
    def GetMod(self,get):
        ...
        rep = "[\xe2\x94\x9c\xe2\x94\x80\xe2\x94\x80]"
        modList = re.sub(rep,'',tmp[0]).split();
        result = []
        for m in modList:
            mod = {}
            tmp = m.split('@');
...
''' % (version,version,version)
    ...
    def Add(self,get):
        ...
        if os.path.exists(get.path + '/package.json') and not os.path.exists(get.path + '/package-lock.json'): os.system(self.__SR + "cd " + get.path + ' && npm install');
