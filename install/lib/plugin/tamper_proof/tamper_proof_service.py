...
class MyEventHandler(pyinotify.ProcessEvent):
    ...
    def exclude_PATH(self,pathname):
        ...
        siteData = self.get_SITE_CONFIG(pathname)
        if siteData:
            siteData['excludePath'].append('.well-known')
            siteData['excludePath'].append('addons')
            siteData['excludePath'].append('attachment')
            siteData['excludePath'].append('framework')
            for ePath in siteData['excludePath']:
                if ePath.find('/') != -1:
                    if pathname.find(ePath) == 0: return True
                elif ePath in dirNames: return True
...
def run():
    ...
    for siteInfo in sites:
        ...
        watchManager.add_watch(siteInfo['path'].encode('utf-8'), mode , rec=True)
